...
const {totalCartItems,totalItems,getCartTotal,user,logout} = useContext(AuthContext)
const toggleNav = ()=>{
  setToggle(!toggle)
} 
              ...
              <div>
                {
                    Object?.keys(user).length > 0
                    &&
                      <button className="font-bold bg-red-700 text-white px-1 rounded-md" onClick={logout}>Logout</button>
                 }
              </div>
            </div>
